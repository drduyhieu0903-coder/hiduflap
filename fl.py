...
from translation_glossary import CUSTOM_TRANSLATIONS
import re
...
CLINICAL_DIR      = DATA_ROOT / "_clinical"
SOURCE_PDF_DIR    = DATA_ROOT / "_source_pdfs" # <-- THÊM DÒNG NÀY
ensure_dir = lambda p: Path(p).mkdir(parents=True, exist_ok=True)
ensure_dir(SOURCE_PDF_DIR) # <-- THÊM DÒNG NÀY ĐỂ TỰ TẠO THƯ MỤC
ensure_dir = lambda p: Path(p).mkdir(parents=True, exist_ok=True)
...
def translate_vietnamese_to_english(text: str) -> str:
    """
    Dịch văn bản tiếng Việt sang tiếng Anh, ưu tiên dùng từ điển tùy chỉnh.
    """
    if not text.strip():
        return ""
...
def render_image_card_lightweight(row: pd.Series, df: pd.DataFrame, col_key: str):
    """
    Card ảnh siêu nhẹ để render nhanh hơn, đã được sửa lỗi key trùng lặp TRIỆT ĐỂ.
    """
    rel = row["image_path"]
    # Tạo safe key từ đường dẫn (không dùng hash để tránh collision)
    ...
    🔧          **Loại vạt:** {flap.capitalize()}
        ...
        with col1:
            ...
            # KEY DUY NHẤT: col_key + safe_key (KHÔNG dùng hash)
            st.checkbox("Chọn", value=is_selected, key=f"cb_{col_key}_{safe_key}", 
                        on_change=toggle_image_selection, args=(rel,))
        with col2:
...
def render_selection_strip_optimized(sel_list: List[str], current_page_images: List[str]):
    """Thanh lựa chọn được tối ưu, không hiển thị thumbnail."""
    if not sel_list:
        return
    ...
    with st.container(border=True):
        ...
        btn_label = "Bỏ chọn trang này" if all_on_page_selected else "Chọn cả trang"
        if col2.button(btn_label, key="toggle_page"):
            if all_on_page_selected:
                st.session_state.selected_list = [x for x in sel_list if x not in current_page_images]
            else:
                st.session_state.selected_list = sorted(list(set(sel_list + current_page_images)))
            st.rerun()
...
def toggle_image_selection(image_path: str):
    ...
    Nó cập nhật danh sách selected_list trong session state mà không gây rerun toàn trang.
    """
    if image_path in st.session_state.selected_list:
        # Nếu ảnh đã có trong danh sách, loại bỏ nó (khi người dùng bỏ chọn)
        st.session_state.selected_list.remove(image_path)
    else:
        # Nếu ảnh chưa có, thêm nó vào (khi người dùng chọn)
